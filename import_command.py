...
from eab_utils.logger import LOG
...
class ImportCommand:
    ...
    def parse_argsv(self, argv):
        ...
        parser = argparse.ArgumentParser(
            description="This command allow you to import objects or scene to a blend file and save it once done."
        )

        file_group = parser.add_argument_group("File path")
        file_group.add_argument(
            "-f", "--source_file", nargs="?", help="Path to blend file to export data from", required=True
        )
        file_group.add_argument(
            "-d", "--destination_file", nargs="?", help="Path to blend file you want the export data to", required=True
        )

        import_option_group = parser.add_argument_group("Import Options")
        import_option_group.add_argument(
            "-s",
            "--source_data",
            choices=["OBJECTS", "SCENE"],
            default="OBJECTS",
        ...
        import_option_group.add_argument(
            "-C",
            "--export_object_children",
            default=False,
            help="if enabled, all listed object children will be exported",
            required=True,
        )
        import_option_group.add_argument(
            "-S", "--source_scene_name", help="The name of the scene from which export data from", required=True
        )
        import_option_group.add_argument("-O", "--source_object_list", nargs="+", help="List of objects to export")

        collection_hierarchy_group = parser.add_argument_group("Collection Hierarchy")
        collection_hierarchy_group.add_argument(
            "-c",
        ...
        name_collision_group.add_argument(
            "-i",
            "--imported_names",
            nargs="+",
        ...
        debug_group.add_argument(
            "-P", "--print_debug", default=False, help="Print debug message in console", required=False
        )
    ...
    def init_source_lists(self):
        self.log.info("Init Source List")
        self.log.info("register previously loaded libraries")
        # register previously loaded libraries
        self.previous_library_objects = {
            o.name: o
            for o in bpy.data.objects
            if o.library is not None and self.conform_path(o.library.filepath) == self.conform_path(self.source_file)
        }
        self.previous_library_collections = {
            c.name: c
            for c in bpy.data.collections
            if c.library is not None and self.conform_path(c.library.filepath) == self.conform_path(self.source_file)
        }
        self.previous_library_scenes = {
            s.name: s
            for s in bpy.data.scenes
            if s.library is not None and self.conform_path(s.library.filepath) == self.conform_path(self.source_file)
        }
        ...
        self.log.info("register imported library datas")
        ...
        self.log.info("register object children")
            ...
            for c in self.objects_children.values():
                for cc in c:
                    if cc not in objects.keys():
                        ...
                        if o.library is not None and os.path.normpath(o.library.filepath) == os.path.normpath(
                            self.source_file
                        ):
                            objects[cc] = o

        self.log.info("register object depencencies")
        ...
        self.log.info("register parent collections")
        ...
        self.log.info("register root collection name")
        ...
        self.collections_in_scene = [
            c.name for c in collections.values() if scenes[self.source_scene_name].user_of_id(c)
        ]

        self.log.info("register object_collection_hierarchy")
        # register object_collection_hierarchy
        self.objects_collection_hierarchy = self.get_objects_collection_hierarchy(
            [o for o in objects.values() if o.name in self.source_object_list],
            self.collections_in_scene,
            collections.values(),
        )

        self.log.info("register objects collection list")
        ...
        self.log.info("register all objects collection hierarchy")
        ...
        self.log.info("remove objects and collections")
    ...
    def import_scene(self):
        ...
        # Import Scene
        bpy.ops.wm.append(
            filepath=filepath, directory=directory, filename=self.source_scene_name, link=self.export_mode == "LINK"
        )
    ...
    def import_objects(self):
        if self.target_scene != "ACTIVE_SCENE":
            if self.target_scene not in bpy.data.scenes:
                self.log.error(
                    f'The target scene "{self.target_scene}" doesn\'t exists in the file, the objects will be placed in the current scene'
                )
            else:
        ...
        self.link_objects(
            ...
            bpy.data.collections[IMPORT_COLLECTION_NAME],
            self.export_mode == "LINK",
        )

        self.have_dependencies = len(bpy.data.collections[IMPORT_COLLECTION_NAME].objects) > len(
            self.source_object_list
        )
    ...
    def create_and_link_to_new_collection(self):
        if self.new_collection_name == "":
            ...
            self.cm.link_collection_to_collection(
                bpy.data.collections[self.new_collection_name], self.scene_root_collection
            )
            for o in self.imported_objects.values():
    ...
    def link_objects(self, blend_file, object_names, collection, is_link):
        ...
        if self.export_object_children:
            self.ordered_children = self.reorder_list_child_first(
                list(self.imported_objects.values()), include_parent=True
            )
    ...
    def get_parent_collection_names(self, collection, parent_names, collection_data):
        ...
        for parent_collection in collection_data:
            if collection.name in parent_collection.children.keys():
                ...
                self.get_parent_collection_names(parent_collection, parent_names, collection_data)
                return
    ...
    def remove_source_library(self):
        if (
            len(self.previous_library_objects)
            or len(self.previous_library_collections)
            or len(self.previous_library_scenes)
        ):
            objects = [
                o
                for o in bpy.data.objects
                if o.library is not None
                and self.conform_path(o.library.filepath) == self.conform_path(self.source_file)
                and o not in self.previous_library_objects.values()
            ]
            collections = [
                c
                for c in bpy.data.collections
                if c.library is not None
                and self.conform_path(c.library.filepath) == self.conform_path(self.source_file)
                and c not in self.previous_library_collections.values()
            ]
            scenes = [
                s
                for s in bpy.data.scenes
                if s.library is not None
                and self.conform_path(s.library.filepath) == os.path.normpath(self.source_file)
                and s not in self.previous_library_scenes.values()
            ]
