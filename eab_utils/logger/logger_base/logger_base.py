...
from .logger_const import MessageType, default_color


def get_log_file():

    log_file = "EAB_" + time.strftime(f"%Y%m%d") + ".log"
    log_file = path.join(tempfile.gettempdir(), log_file)

    print("EAB : Log file path :", log_file)
...
class Logger:
    ...
    def info(self, message, skip_prefix=False, color=None):
        ...
        if color is not None:
            ...
            self.messages.append(message=message, color=Color(default_color))
